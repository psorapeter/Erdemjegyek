...
def tan_stat():
    ...
    for i in range(result.shape[0]):
        if (result.iloc[i]['Márk'] == max_value[i]):
            ossz_tantargy_egyben.tag_add('red', f"{i + 2}.26", f"{i + 2}.30")
        if (result.iloc[i]['Petra'] == max_value[i]):
            ossz_tantargy_egyben.tag_add('red', f"{i + 2}.32", f"{i + 2}.36")
        if (result.iloc[i]['Dóra'] == max_value[i]):
            ossz_tantargy_egyben.tag_add('red', f"{i + 2}.38", f"{i + 2}.end")
    ...
    ossz_tantargy_atlagok = tk.Text(ossz_tantargy_atlagok_frame, width=37,
