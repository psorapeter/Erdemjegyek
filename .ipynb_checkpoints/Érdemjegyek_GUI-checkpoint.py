# pyinstaller --onefile --ico Érdemjegyek_uj\GUI\Érdemjegyek_GUI.ico Érdemjegyek_uj\GUI\Érdemjegyek_GUI.py
...
class Gyerek:
    ...
    def lekerdezes(self):
        global stat_text, stat_entry
        # lekérdezés főablak
        lekerdezes_ablak = tk.Toplevel(root)
        lekerdezes_ablak.title(f'Lekérdezés - {self.nev} - 2023/2024 tanév')
        ...
        treeview_leker.column("Tantárgy", width=150, anchor="center")
        ...
        lekerdezes_ablak_jegyek_frame.grid(row=0, column=1, padx=(5, 10), pady=(10, 5), sticky="news")
        ...
        results_text_leker.pack(padx=5, pady=5)
        results_text_leker.config(state="disabled")
        results_text_leker.delete(1.0, tk.END)
        ...
        lekerdezes_ablak_stat_frame = LabelFrame(lekerdezes_ablak_foframe, borderwidth=1, relief=RIDGE,
                                                 text=f"Statisztikák")
        lekerdezes_ablak_stat_frame.grid(row=0, column=2, rowspan=2, padx=(5, 10), pady=10)
        ...
        stat_entry = tk.Entry(lekerdezes_ablak_stat_frame)
        stat_entry.pack(pady=(5, 10))
        stat_entry.bind("<KeyRelease>", lambda event: find())
        stat_text.config(state="disabled")
    ...
    def havi(self, aktho):
        ...
        treeview_havi.column("Tantárgy", width=150, anchor="center")
...
class Window:
    def __init__(self, master):
        ...
        self.treeview_alap.column("Tantárgy", width=150, anchor="center")
        ...
        self.kereses_tablazat.column("Tantárgy", width=150, anchor="center")
    ...
    def frissit(self):
        ...
        ossz = sum(x for x in (Mark.zsebpenz, Petra.zsebpenz, Dora.zsebpenz) if x > 0)
    ...
    def insert_row(self):
        ...
        try:
            ...
            if "20230901" <= datum <= "20240630":
                datum = date_obj.strftime('%Y.%m.%d')
                datum_helyes = True
            else:
                uzenet("Dátum hiba", "Megengedett tartomány: 2023.09.01 - 2024.06.30!")
        except ValueError:
        ...
        if selected_item:
            ...
            try:
                ...
                if "20230901" <= datum <= "20240630":
                    ...
                    # message="Megengedett tartomány: 2023.09.01 - 2024.06.30!")
            except ValueError:
                uzenet("Dátumm hiba", "Érvénytelen dátumformátum!")
...
def find():
    ...
    stat_text.tag_remove('found', '1.0', END)
    ...
    if (s):
        ...
        while 1:
            ...
            idx = stat_text.search(s, idx, nocase=1,
                                   stopindex=END)
            ...
            stat_text.tag_add('found', idx, lastidx)
            idx = lastidx

            # mark located string as red

        stat_text.tag_config('found', foreground='red')
    stat_entry.focus_set()
...
Mark = Gyerek('Márk', 8000)
Mark.tantargyak = ["Angol nyelv", "Biológia", "Digitális kultúra", "Ének-zene", "Etika/Hit- és erkölcstan", "Fizika",
                   "Földrajz", "Irodalom", "Kémia", "Magatartás", "Szorgalom", "Magyar nyelv", "Matematika",
                   "Testnevelés", "Történelem", "Vízuális kultúra"]
Petra = Gyerek('Petra', 5000)
Petra.tantargyak = ["Angol nyelv", "Digitális kultúra", "Dráma és színház", "Ének-zene", "Etika/Hit és erkölcstan",
                    "Irodalom", "Magatartás", "Szorgalom", "Magyar nyelv", "Matematika", "Technika és tervezés",
                    "Természettudomány", "Testnevelés", "Történelem", "Vízuális kultúra"]
Dora = Gyerek('Dóra', 2000)
Dora.tantargyak = ["Ének-zene", "Etika", "Irodalom", "Magatartás", "Szorgalom", "Magyar nyelv", "Matematika",
                   "Technika és tervezés", "Testnevelés", "Vízuális kultúra"]
...
# Ablak létrehozása
root = tk.Tk()
root.title("Érdemjegyek, havi zsebpénzek - 2023/2024 tanév")
